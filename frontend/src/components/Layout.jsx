...
const Layout = () => {
  ...
  // Función que manda un mensaje de advertencia cuando un usuario que no tiene permiso de acceder a una pantalla
  // intenta acceder
    ...
    <div className="app-container" style={{ display: 'flex', height: '100vh', width: '100vw' }}>
      ...
      <aside className="app-sidebar">
        ...
        <div className="sidebar-menu">
          {/* Acceso con todos los roles */}
          <i className={`bi bi-house-door ${location.pathname === '/dashboard' ? 'active' : ''}`}
            onClick={() => handleNavigation('/dashboard')} title="Inicio"></i>

          {/* Acceso solo con ADMIN y ODONTOLOGO */}
          <i className={`bi bi-people ${location.pathname === '/pacientes' ? 'active' : ''}`}
            onClick={() => handleNavigation('/pacientes', ['ADMIN', 'ODONTOLOGO'])} title="Pacientes"></i>

          {/* Acceso solo con ADMIN y RECEPCIONISTA */}
          <i className={`bi bi-calendar3 ${location.pathname === '/agenda' ? 'active' : ''}`}
            onClick={() => handleNavigation('/agenda', ['ADMIN', 'RECEPCIONISTA'])} title="Agenda"></i>

          {/* Acceso solo con ADMIN */}
          <i className={`bi bi-person-badge ${location.pathname === '/usuarios' ? 'active' : ''}`}
            onClick={() => handleNavigation('/usuarios', ['ADMIN'])} title="Personal de Clinica"></i>

          {/* Acceso solo con ADMIN y ODONTOLOGO */}
          <i className={`bi bi-tooth ${location.pathname === '/odontograma' ? 'active' : ''}`}
            onClick={() => handleNavigation('/odontograma', ['ADMIN', 'ODONTOLOGO'])} title="Odontograma"></i>
        </div>
